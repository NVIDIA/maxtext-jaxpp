...
def validate_attention_kernel(s: str) -> None:
  valid_attention_kernels = ("autoselected", "dot_product", "flash", "cudnn_flash_te", "jaxpp_cudnn_stablehlo", "jaxpp_te")
  ...
  def user_init(raw_keys):
    ...
    if using_pipeline_parallelism(raw_keys):
      ...
      if not raw_keys['use_jaxpp']:
        ...
        assert isinstance(raw_keys['num_pipeline_repeats'], int) and raw_keys['num_pipeline_repeats'] != -1, f"num_pipeline_repeats is mandatory when use_jaxpp=True, found: {raw_keys['num_pipeline_repeats']}"
      if raw_keys['num_pipeline_microbatches'] == -1:
        raw_keys['num_pipeline_microbatches'] = num_stages
      assert raw_keys['num_pipeline_microbatches'] % num_stages == 0 or raw_keys["use_jaxpp"], f"The number of microbatches ({raw_keys['num_pipeline_microbatches']}) must be divisible by the number of stages ({num_stages})"
